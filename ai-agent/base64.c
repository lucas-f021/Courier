...
int b64_decode(const char *in, unsigned char *out, size_t out_size, size_t *out_len) {
    *out_len = 0; // init out_len
    while(in[0] != '\0') { // while in is not null term
        ...
        if (v1 < 0 || v2 < 0) return -1; // invalid base64 input

        if (*out_len + 1 > out_size) return -1; // buffer overflow guard
